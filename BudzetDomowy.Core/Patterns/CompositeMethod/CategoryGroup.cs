...
{
    // Kompozyt (Composite) - Grupa Kategorii (np. "Dom", "Jedzenie").
    // Przechowuje listę dzieci i deleguje do nich operacje (np. sumowanie).
    public class CategoryGroup : CategoryComponent
    {
        ...
        {
            // Rekurencyjne sumowanie wydatków wszystkich dzieci
            double sum = 0;
            foreach (var child in _children) sum += child.GetTotalAmount();
            return sum;
        }
